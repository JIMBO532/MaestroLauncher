...
fetch that release's installer directly.
...
Downloading the installer is a real action with a real failure mode, so
that part -- unlike the check -- raises ``UpdateError`` the normal way.
"""
...
def download_installer(
    ...
    ``url`` is not re-validated against ``INSTALLER_ASSET_NAME`` here --
    that check belongs to whoever decided to call this at all (the GUI
    restricts it to the URL its own update check handed back).
    """
